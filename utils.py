...
def unmold_embedding(embedding, bbox, image_shape):
    """Converts a predicted embedding into a full size embedding map.
    embedding: [height, width, depth] of type float. A small, typically 28x28 embedding.
    bbox: [y1, x1, y2, x2]. The box to fit the embedding in.
    Returns a full size embedding map with the same size as the original image.
    """
    y1, x1, y2, x2 = bbox
    print(f"Bounding box: {bbox}")

    # Resize the embedding to the size of the bounding box
    target_height = y2 - y1
    target_width = x2 - x1
    resized_embedding = cv2.resize(embedding, (target_width, target_height), interpolation=cv2.INTER_LINEAR)

    # Debug: Print the resized embedding
#    print(f"-------------------Resized embedding {embedding.shape} -> ({target_width}, {target_height})-------------------")
#    print(resized_embedding)

    # Initialize the full embedding map
    full_embedding = np.zeros((image_shape[0], image_shape[1], embedding.shape[-1]), dtype=np.float32)

    # Ensure the coordinates are within the bounds of the full_embedding
    y1 = max(0, y1)
    x1 = max(0, x1)
    y2 = min(image_shape[0], y2)
    x2 = min(image_shape[1], x2)

    # Calculate the dimensions to place the resized embedding
    target_y = min(y2 - y1, resized_embedding.shape[0])
    target_x = min(x2 - x1, resized_embedding.shape[1])

    # Debug: Print the target placement dimensions
#    print(f"Target placement dimensions: target_y = {target_y}, target_x = {target_x}")

    # Ensure the resized embedding fits within the target placement dimensions
    resized_embedding = resized_embedding[:target_y, :target_x, :]

    # Place the resized embedding in the correct location
    try:
#        print(f"Placing resized embedding at: y1={y1}, y1+target_y={y1+target_y}, x1={x1}, x1+target_x={x1+target_x}")
        full_embedding[y1:y1 + target_y, x1:x1 + target_x, :] = resized_embedding
    except ValueError as e:
        print(f"Error placing resized embedding into full embedding: {e}")
        print(f"Resized embedding shape: {resized_embedding.shape}, full embedding shape: {full_embedding.shape}")
        print(f"Indices: y1:{y1}, y1+target_y:{y1+target_y}, x1:{x1}, x1+target_x:{x1+target_x}")

    # Debug: Print a section of the full embedding map to verify
    print(f"-------------------Section of Full embedding map-------------------")
    print(full_embedding[y1:y1 + target_y, x1:x1 + target_x, :])

    # Debug: Check if the full embedding map contains non-zero values
    non_zero_count = np.count_nonzero(full_embedding)
    print(f"Non-zero count in full embedding map: {non_zero_count}")

    return full_embedding[y1:y1 + target_y, x1:x1 + target_x, :]
