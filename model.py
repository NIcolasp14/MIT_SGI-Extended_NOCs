...
class MaskRCNN(nn.Module):
    ...
    def detect(self, images):
        ...
        embeddings: [H, W, N, D] feature embeddings from the final layer
        """
        ...
        # Run object detection and get embeddings too
        detections, mrcnn_mask,mrcnn_coord_x,mrcnn_coord_y,mrcnn_coord_z, embeddings = self.predict([molded_images, image_metas], mode='inference')
        ...
        embeddings = embeddings.permute(0, 2, 3, 1).data.cpu().numpy() # premute embeddings as well
        ...
        for i, image in enumerate(images):
            # unmold embeddings
            final_rois, final_class_ids, final_scores, final_masks, final_coords, final_embeddings =\
                self.unmold_detections(detections[i], mrcnn_mask[i], mrcnn_coord[i],
                                       embeddings[i], image.shape, windows[i])
            results.append({
                "rois": final_rois,
                ...
                "masks": final_masks,
                "coords":final_coords,
                "embeddings": final_embeddings # save embedidngs
            })
        return results
    ...
    def predict(self, input, mode):
        ...
        if mode == 'inference':
            ...
            # Capture final layer's embeddings
            embeddings = mrcnn_feature_maps[-1]
#            print("-------------------embeddings-------------------")
#            print(embeddings)
            
            return [detections, mrcnn_mask,mrcnn_coord_x_bin_value,mrcnn_coord_y_bin_value,mrcnn_coord_z_bin_value, embeddings]
    ...
    def unmold_detections(self, detections, mrcnn_mask, mrcnn_coord, embeddings, image_shape, window):
        ...
        embeddings: [N, common_height, common_width, depth] Cropped embeddings
        """
        print("-------------------Initial Embeddings-------------------") # debugging
        print(embeddings)
        ...
        embeddings = np.repeat(embeddings[np.newaxis, :, :, :], N, axis=0) # repeat embeddings for each detection
        
        print("-------------------Repeated Embeddings-------------------") # debugging
        print(embeddings)
        ...
        if exclude_ix.shape[0] > 0:
            ...
            embeddings = np.delete(embeddings, exclude_ix, axis=0)
            N = class_ids.shape[0]

        # Define a common shape for cropped embeddings
        common_height = max([box[2] - box[0] for box in boxes])
        common_width = max([box[3] - box[1] for box in boxes])
        ...
        for i in range(N):
            ...
            full_embedding = utils.unmold_embedding(embeddings[i], boxes[i], image_shape)

            # Resize cropped embedding to common shape
            resized_embedding = cv2.resize(full_embedding, (common_width, common_height), interpolation=cv2.INTER_LINEAR)
            full_embeddings.append(resized_embedding)

            print(f"-------------------Unmolded embedding {i}-------------------")
            print(full_embedding)
        
        full_masks = np.stack(full_masks, axis=-1) if full_masks else np.empty((0,) + masks.shape[1:3])
        full_coords = np.stack(full_coords, axis=-2) if full_coords else np.empty((0,) + coords.shape[1:3])
        full_embeddings = np.stack(full_embeddings, axis=0) if full_embeddings else np.empty((0, common_height, common_width, depth))
        
        print("-------------------Full Embeddings-------------------")
        print(full_embeddings)
        
        return boxes, class_ids, scores, full_masks, full_coords, full_embeddings
